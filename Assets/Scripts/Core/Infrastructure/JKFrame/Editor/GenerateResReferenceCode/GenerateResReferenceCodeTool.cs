...
{
    private static readonly string scriptPath = Application.dataPath + "/JKFrame/R.cs";
...
~~成员~~
    ...
    private static readonly string SubAssetPropertyTemplate =
        @"  
        public static ##类型## ##资源名称## { get => ResSystem.LoadAsset<##类型##>(""##资源路径##""); }";
    private static readonly string GameObjectPropertyTemplate =
        @"  
    ...
    public static void GenerateResReferenceCode()
    {
        ...
        FileStream file = new(scriptPath, FileMode.CreateNew);
        StreamWriter fileW = new(file, Encoding.UTF8);

        HashSet<string> allTypeAssemblyNames = new();
        ...
        foreach (AddressableAssetGroup group in assets.groups)
        {
            ...
            // 找到子类全部资源以及类型
            List<AddressableAssetEntry> allAssetEntry = new();
