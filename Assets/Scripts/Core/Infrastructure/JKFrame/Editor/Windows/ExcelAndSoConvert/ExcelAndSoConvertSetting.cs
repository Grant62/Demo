...
namespace JKFrame.Editor
{
    ...
    {
        private const int typeIndex = 1; // 类型
        private const int fieldNameIndex = 2; // 变量名
        private const int remarkIndex = 3; // 备注
        ...
        [TabGroup("生成Excel模板")] [LabelText("文件名(空则使用class名)")]
        public string fileName;
        [TabGroup("生成Excel模板")] [LabelText("生成类型")] [EnumToggleButtons]
        public Type generateExcelTemplateClassType;
        [TabGroup("生成Excel模板")] [LabelText("使用OdinLabelText进行备注")]
        public bool useLableText;
        [TabGroup("生成Excel模板")] [LabelText("List<T>集合模式")] [Tooltip("用于一张表对应一个SO集合的方式")]
        public bool useListFied;
        [TabGroup("生成Excel模板")] [LabelText("List<T>字段名称")] [ShowIf("CheckGenerateExcelTemplateFieldName")]
        public string generateExcelTemplateFieldName;

        private bool CheckGenerateExcelTemplateFieldName { get => useListFied; }
        ...
        private void GenerateExcelTemplateFile(string generateExcelTemplatePath, string fileName, Type generateExcelTemplateClassType, bool useLableText, bool useListFied,
            string generateExcelTemplateFieldName)
        {
        ...
        [TabGroup("批量转换")] [LabelText("要转的类")] [ShowIf("CheckConversionTye")]
        public Type conversionTye;
        [InfoBox("True:使用GUID进行资源记录（也就是资源位置不影响表格），False:使用路径进行记录")]
        [TabGroup("批量转换")] [LabelText("GUIDMode")]
        public bool guidMode;
        ...
        [TabGroup("批量转换")] [LabelText("SO文件名采用前缀_数字的形式")] [ShowIf("CheckUseSoFileNamePrefix")]
        public bool useSoFileNamePrefix;
        [TabGroup("批量转换")] [LabelText("SO文件名使用第几个字段")] [ShowIf("CheckSoFileNameUseFileIndex")]
        public int soFileNameUseFileIndex;
        [TabGroup("批量转换")] [LabelText("SO文件名前缀")] [ShowIf("CheckSoFileNamePrefixName")]
        public string soFileNamePrefixName;

        [TabGroup("批量转换")] [LabelText("SO文件")] [ShowIf("CheckSoFile")]
        public ScriptableObject soFile;

        [TabGroup("批量转换")] [LabelText("SO中List变量的名称")] [ShowIf("CheckListFieldName")]
        public string listFieldName;
        [TabGroup("批量转换")] [LabelText("转换前清空SO文件夹目录")] [ShowIf("CheckDeleteFolder")]
        ...
        private void Excel2OneSO()
        {
            ...
            // 读取表格数据，填充进SO
            // 根据表头找到所有的FieldInfo
            using (ExcelPackage excelPackage = new(new FileStream(excelFilepath, FileMode.Open)))
            {
                ...
                for (int x = 1; x <= colCount; x++)
                {
                    ...
                    if (!allEmpty)
                    {
                        addMethodInfo.Invoke(list, new[] { obj });
                    }
                }
            }
        ...
        private void Excel2SOs()
        {
            ...
            using (ExcelPackage excelPackage = new(new FileStream(excelFilepath, FileMode.Open)))
            {
                ...
                for (int x = 1; x <= colCount; x++)
                {
                    ScriptableObject obj = CreateInstance(conversionTye);
                    bool allEmpty = true; // 一行全是空则说明可以停止
                    // 列
                    for (int y = 1; y <= fieldInfoList.Count; y++)
                    {
        ...
        private void OneSo2Excel()
        {
            ...
            FileStream fileStream = new(excelFilepath, FileMode.CreateNew);
        ...
        private void SOs2Excel()
        {
            ...
            using (ExcelPackage excelPackage = new(fileStream))
            {
                ...
                for (int i = 0; i < soFilePaths.Length; i++)
                {
                    ...
                    // 挨个写入
                    Object obj = AssetDatabase.LoadAssetAtPath(path, conversionTye);
                    if (obj != null)
                    {
                        for (int x = 0; x < fieldInfos.Length; x++)
                        {
        ...
        private bool CheckConversionTye { get => conversion == Conversion.Excel2SOs || conversion == Conversion.SOs2Excel; }

        private bool CheckConversionExcelFilePath { get => conversion == Conversion.Excel2OneSO || conversion == Conversion.Excel2SOs; }

        private bool CheckUseSoFileNamePrefix { get => conversion == Conversion.Excel2SOs; }

        private bool CheckConversionSoFolderPath { get => conversion == Conversion.Excel2SOs || conversion == Conversion.SOs2Excel; }

        private bool CheckDeleteFolder { get => conversion == Conversion.Excel2SOs; }

        private bool CheckListFieldName { get => conversion == Conversion.Excel2OneSO || conversion == Conversion.OneSo2Excel; }

        private bool CheckSoFileNameUseFileIndex { get => CheckConversionSoFolderPath && !useSoFileNamePrefix && conversion != Conversion.SOs2Excel; }

        private bool CheckSoFileNamePrefixName { get => !CheckSoFileNameUseFileIndex && conversion == Conversion.Excel2SOs; }

        private bool CheckSoFile { get => conversion == Conversion.Excel2OneSO || conversion == Conversion.OneSo2Excel; }

        private bool CheckSaveExcelFilePath { get => conversion == Conversion.OneSo2Excel || conversion == Conversion.SOs2Excel; }

        private bool CheckSaveExcelName { get => conversion == Conversion.OneSo2Excel || conversion == Conversion.SOs2Excel; }

        private bool CheckSaveExcelFileFolderPath { get => conversion == Conversion.OneSo2Excel || conversion == Conversion.SOs2Excel; }
        #endregion

        #region 工具函数
        private Object GetSubAsset(string subAssetName, string assetPath, bool guidMode)
        {
            if (guidMode)
            {
                // 获取原始路径
        ...
        private void SetDataField(object obj, FieldInfo fieldInfo, string excelValue)
        {
            ...
            if (fieldInfo.FieldType.IsSubclassOf(typeof(Object)))
            {
                ...
                if (isSubAsset)
                {
                    ...
                    Object asset = GetSubAsset(paths[1], paths[0], guidMode);
                    fieldInfo.SetValue(obj, asset);
                }
                else
                {
                    if (guidMode) path = AssetDatabase.GUIDToAssetPath(excelValue);
                    Object unityeObject = AssetDatabase.LoadAssetAtPath(path, fieldInfo.FieldType);
        ...
        private string GetDataField(object obj, FieldInfo fieldInfo, bool guidMode)
        {
            ...
            if (fieldInfo.FieldType.IsSubclassOf(typeof(Object)))
            {
                if (data != null)
                {
                    Object unityeObject = (Object)data;
