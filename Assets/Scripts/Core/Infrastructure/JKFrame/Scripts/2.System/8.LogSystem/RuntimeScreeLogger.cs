...
public class RuntimeScreeLogger : MonoBehaviour
{
    ...
    private readonly List<LogMessage> logs = new();
    ...
    public void Init()
    {
        ...
        Color lineColor = new(0, 0, 0, 0.75f);
    ...
    private readonly float stackTraceHeight = 200;
    ...
    private void OnGUI()
    {
        ...
        if (show)
        {
            ...
            for (int i = 0; i < logs.Count; i++)
            {
                ...
                if (logMessage.logType == LogType.Log && !showLog) continue;
                if (logMessage.logType == LogType.Warning && !showWarning) continue;
                if (logMessage.logType == LogType.Error && !showError) continue;
