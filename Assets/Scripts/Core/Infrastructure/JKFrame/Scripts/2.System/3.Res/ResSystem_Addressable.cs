...
namespace JKFrame
{
    public static class ResSystem
    {
        ...
        public static void InstantiateGameObjectAsync(Transform parent, string keyName, Action<GameObject> callback, bool autoRelease = true)
        {
            GameObject go;
            go = PoolSystem.GetGameObject(keyName, parent);
            if (!go.IsNull()) callback?.Invoke(go);
            else
            {
                Addressables.InstantiateAsync(keyName, parent).Completed += handle => { OnInstantiateGameObjectAsyncCompleted(handle, callback, keyName, autoRelease); };
            }
        }
        ...
        private static void OnInstantiateGameObjectAsyncCompleted<T>(AsyncOperationHandle<GameObject> handle, Action<T> callback, string gameObjectName, bool autoRelease =
        ...
        public static T InstantiateGameObject<T>(Transform parent, string keyName, bool autoRelease = true) where T : Component
        {
            ...
            if (!go.IsNull()) return go.GetComponent<T>();
            return null;
        }
        ...
        /// <param name="parent">父物体</param>
        /// <param name="keyName">对象池中的分组名称，可为Null</param>
        public static void InstantiateGameObjectAsync<T>(string assetName, Action<T> callback = null, Transform parent = null, string keyName = null, bool autoRelease =
            true) where T : Component
        {
            GameObject go;
            if (keyName == null) go = PoolSystem.GetGameObject(assetName, parent);
        ...
        public static void LoadAssetAsync<T>(string assetName, Action<T> callback) where T : Object
        {
            Addressables.LoadAssetAsync<T>(assetName).Completed += handle => { OnLoadAssetAsyncCompleted(handle, callback); };
        }

        private static void OnLoadAssetAsyncCompleted<T>(AsyncOperationHandle<T> handle, Action<T> callback) where T : Object
        {
            callback?.Invoke(handle.Result);
        }
        ...
        public static IList<T> LoadAssets<T>(string keyName, out AsyncOperationHandle<IList<T>> handle, Action<T> callBackOnEveryOne = null) where T : Object
        {
            handle = Addressables.LoadAssetsAsync(keyName, callBackOnEveryOne, true);
        ...
        public static void LoadAssetsAsync<T>(string keyName, Action<AsyncOperationHandle<IList<T>>> callBack, Action<T> callBackOnEveryOne = null) where T : Object
        {
            Addressables.LoadAssetsAsync(keyName, callBackOnEveryOne).Completed += callBack;
        }
