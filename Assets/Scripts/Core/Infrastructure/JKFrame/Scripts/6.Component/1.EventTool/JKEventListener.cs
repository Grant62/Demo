...
namespace JKFrame
{
    ...
    {
        private static readonly ObjectPoolModule poolModul = new();
        ...
        {
            public readonly Dictionary<int, IJKEventListenerEventInfos> eventInfoDic = new();
        }
        ...
        private class JKEventListenerEventInfo<T, TEventArg> : IJKEventListenerEventInfo<T>
        {
            ...
            public void Init(Action<T, TEventArg> action, TEventArg args = default)
            {
                this.action = action;
                arg = args;
            }
        ...
        private interface IJKEventListenerEventInfos
        {
            void RemoveAll();
        }
        ...
        {
            // 所有的事件
            private readonly List<IJKEventListenerEventInfo<T>> eventList = new();
            ...
            ///     添加事件 有参
            /// </summary>
            public void AddListener<TEventArg>(Action<T, TEventArg> action, TEventArg args = default)
            {
            ...
            ///     同一个函数+参数注册过多次，无论如何该方法只会移除一个事件
            /// </summary>
            public void RemoveListener<TEventArg>(Action<T, TEventArg> action, TEventArg args = default)
            {
                for (int i = 0; i < eventList.Count; i++)
                {
        ...
        {
            TriggerAction((int)eventType, eventData);
        }
