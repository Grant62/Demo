...
namespace JKFrame
{
    ...
    public class JKFrameSetting : SerializedScriptableObject
    {
        ...
        [LabelText("日志设置")] public LogSetting LogConfig = new();
        ...
        {
            [LabelText("启用日志")] [OnValueChanged("EnableLogValueChaged")]
            public bool enableLog = true;
            [LabelText("写入时间")] [OnValueChanged("EnableLogValueChaged")]
            public bool writeTime = true;
            [LabelText("写入线程ID")] [OnValueChanged("EnableLogValueChaged")]
            public bool writeThreadID = false;
            [LabelText("写入堆栈")] [OnValueChanged("EnableLogValueChaged")]
            public bool writeTrace = true;
            [LabelText("保存日志文件")] [OnValueChanged("EnableLogValueChaged")]
            public bool enableSave = false;
            [LabelText("需要保存的日志类型")] [HideIf("CheckSaveState")] [EnumFlags] [OnValueChanged("EnableLogValueChaged")]
            public LogType saveLogTypes;
            [LabelText("保存路径,相对persistentDataPath的路径")] [HideIf("CheckSaveState")] [OnValueChanged("EnableLogValueChaged")]
        ...
        public void SetResourcesSystemType()
        {
            switch (ResourcesSystemType)
            {
                case ResourcesSystemType.Resources:
                    ...
                    if (File.Exists(path)) AssetDatabase.DeleteAsset("Assets/JKFrame//Scripts/2.System/3.Res/R.cs");
                    break;
